...
class MainWidget(RelativeLayout):
    ...
    real_fps = StringProperty('RFPS: 0')
    ...
    def __init__(self, **kwargs):
        ...
        if self.is_desktop():
            self._keyboard = Window.request_keyboard(self, self.keyboard_closed)
    ...
    def update(self, dt):
        ...
        if not self.state_game_over and self.state_game_has_started:
            ...
            while self.current_offset_y >= spacing_y:
                ...
                self.score_txt = f'SCORE: {self.current_y_loop}'
                self.real_fps = f'RFPS: {Clock.schedule_interval(self.rfps_start, 1)}'
                self.generate_tiles_coordinates()
    ...
    def rfps_start(self, dt):
        rfps = ClockBaseBehavior('asyncio')
        return str(rfps.get_rfps())
